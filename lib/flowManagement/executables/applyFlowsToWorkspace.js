...
exports.applyFlowsToWorkspace = async function (args, options, logger) {
    ...
    const [flowFilenames, existingFlows, credentials] = await Promise.all([flowFilenamesPromise, existingFlowsPromise, credentialsPromise]);
    ...
    await Promise.all(flowFilenames.map(async(flowFilename) => {
        ...
        const matchingFlows = existingFlows.filter((flow) =>
            matchType === 'name' ?
        ...
        await Promise.all(flow.graph.nodes.map(async (node) => {
            ...
            if(node.credentials_id && matchType === 'name') {
                const matchingCredentials = credentials.data.filter(c => c.relationships.component.data.id === node.component_id);
