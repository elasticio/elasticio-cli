...
exports.readFixtureFile = function readFixtureFile(componentPath) {
  ...
  while (currentPath !== 'C:\\' && currentPath !== '/') {
    fixtureFile = path.resolve(currentPath, './test/fixture.json');
    if (fs.existsSync(fixtureFile)) break;
  ...
  const data = resolveVars(template);
...
exports.readComponentJSON = function readComponentJSON(componentPath) {
  ...
  const content = fs.readFileSync(path.resolve(componentPath, 'component.json')).toString();
  return content;
};
