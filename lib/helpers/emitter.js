...
exports.Emitter = class Emitter extends EventEmitter {
  constructor() {
    ...
    this.VALID_EVENTS = new Set(['data', 'error', 'snapshot', 'end']);
    this.FIVE_MB = 5000000;
   ...
   */
  emit(eventName, ...args) {
    if (!this.VALID_EVENTS.has(eventName)) throw new Error(`The event type ${eventName} is not recognized`);

    if (eventName === 'data' || eventName === 'snapshot') {
      args.forEach((arg) => {
        if (arg !== undefined && arg !== null && arg.constructor !== Object) throw new Error('The argument for this event must be a JSON');
        if (Emitter.jsonSize(arg) > this.FIVE_MB) throw new Error('The emitted JSON cannot be greater than 5 MB');
      });
    }
