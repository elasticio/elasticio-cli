...
const EXTENDED_VIEW_CLASES = new Set([
  'TextFieldView',
  ...
  if (component.credentials) {
    ...
    if (fields) {
      Object.keys(fields).forEach((field) => {
        ...
        if (viewClass) {
          assert.warn(EXTENDED_VIEW_CLASES.has(fields[field].viewClass), `view class '${fields[field].viewClass}' in credential field '${field}' is not valid
    ...
    try {
      ...
      if (details.fields) {
        Object.keys(details.fields).forEach((f) => {
          ...
          if (field.viewClass) {
            assert.warn(EXTENDED_VIEW_CLASES.has(field.viewClass), `view class '${field.viewClass}' in credential field '${name}' is not valid
                => Valid username classes are [${VIEW_CLASSES}]`);
