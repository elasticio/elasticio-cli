...
const checkFuncDetails = (details, name, pathName, type) => {
  ...
  assert.error(details.viewClass, `${type} '${name}' is missing a viewClass`);
  ...
  if (actionExists) {
    try {
      ...
      if (details.viewClass) {
        assert.warn(EXTENDED_VIEW_CLASES.includes(details.viewClass), `view class '${details.viewClass}' in credential field '${name}' is not valid
            => Valid username classes are [${VIEW_CLASSES}]`);
        if (details.viewClass === 'SelectView') {
          assert.error(typeof details.model === 'object' || (typeof details.model === 'string' && actionFile[details.model]),
            `field '${name}' requires the dropdown for the model contain an object or a function in the ${type} file
            \t=> Your model is: '${details.model}'`);
        }
      }
    } catch (e) {
